package com.ywg.androidcommon.photopicker.ui;
...
import com.ywg.androidcommon.photopicker.ImageDataSource;
import com.ywg.androidcommon.photopicker.ImagePicker;
import com.ywg.androidcommon.photopicker.Utils;
import com.ywg.androidcommon.photopicker.adapter.ImageFolderAdapter;
import com.ywg.androidcommon.photopicker.adapter.ImageGridAdapter;
import com.ywg.androidcommon.photopicker.bean.ImageFolder;
...
public class ImageGridActivity extends ImageBaseActivity implements ImageDataSource.OnImagesLoadedListener, ImageGridAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_ok:
                ...
                okIntent.putExtra(ImagePicker.EXTRA_RESULT_ITEMS, imagePicker.getSelectedImages());
                setResult(ImagePicker.RESULT_CODE_ITEMS, okIntent);  //多选不允许裁剪裁剪，返回数据
                finish();
                break;
            case R.id.btn_dir:
            ...
            case R.id.btn_preview:
                Intent previewIntent = new Intent(ImageGridActivity.this, ImagePreviewActivity.class);
                previewIntent.putExtra(ImagePicker.EXTRA_SELECTED_IMAGE_POSITION, 0);
                previewIntent.putExtra(ImagePicker.EXTRA_IMAGE_ITEMS, imagePicker.getSelectedImages());
                previewIntent.putExtra(ImagePreviewActivity.ISORIGIN, isOrigin);
                startActivityForResult(previewIntent, ImagePicker.REQUEST_CODE_PREVIEW);
                break;
            case R.id.btn_back:
                //点击返回按钮
                finish();
     ...
     * 创建弹出的ListView
     */
    private void createPopupFolderList(int screenWidth, int screenHeight) {
        mFolderPopupWindow = new ListPopupWindow(this);
        //mFolderPopupWindow.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        mFolderPopupWindow.setAdapter(mImageFolderAdapter);
        mFolderPopupWindow.setContentWidth(screenWidth);
        mFolderPopupWindow.setWidth(screenWidth);  //如果不设置，就是 AnchorView 的宽度
        ...
        /*int folderItemViewHeight =
                ...
                getResources().getDimensionPixelOffset(R.dimen.folder_cover_size) +
                        ...
                        // Padding Bottom
                        getResources().getDimensionPixelOffset(R.dimen.folder_padding);*/
        int folderViewHeight = mImageFolderAdapter.getCount() * mImageFolderAdapter.getItemViewHeight();
        ...
        /*int maxHeight = screenHeight * 5 / 8;
